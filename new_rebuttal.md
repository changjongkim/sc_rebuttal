...
We also compare LMCache (Disk) and LMCache (Redis) as KV-system baselines. We tried to add Mooncake by launching its Conductor as a separate batch job, but a persistent coordinator is non-executable under batch scheduling, which is the system issue. IMPRESS is single-node, and its importance-informed loading optimizes prefix reuse within one node, with no cross-node sharing or global deduplication.
