...
- **vLLM and PyTorch (R2, R4).** The objection assumes the gain comes from comparing CASCADE on PyTorch against vLLM. It does not. The gain is measured within one PyTorch run. Fig. 13b shows CASCADE replacing about 1 s of GPU prefill with a 21 to 27 ms RDMA retrieval, both timed in the same runtime. The framework is identical before and after, so it cannot affect the gain, and there is nothing to disentangle. The vLLM numbers in Fig. 13a are a separate reference point, not part of this measurement.
